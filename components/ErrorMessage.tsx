...
export const ErrorMessage: React.FC<ErrorMessageProps> = memo(
  ({ message, onRetry, isChatError = false }) => {
    ...
    const getTooltipContent = (errorMsg: string): string => {
      ...
      if (e.includes('rate') || e.includes('429') || e.includes('busy') || e.includes('503')) return 'Rate limited or service busy. Please try again in a moment.';
      if (e.includes('auth') || e.includes('key') || e.includes('invalid')) return 'Authentication issue. Please check your API key in settings.';
      if (e.includes('model') || e.includes('unsupported') || e.includes('deprecated') || e.includes('404')) return 'Model unavailable or unsupported. Please try updating your model in settings or contact support.';
      if (e.includes('safety') || e.includes('blocked')) return 'The response was blocked due to safety settings. Please try rephrasing your prompt.';
    ...
    return (
      ...
      >
        <div className="flex items-center space-x-2">
          {(message.toLowerCase().includes('safety') || message.toLowerCase().includes('blocked') || message.toLowerCase().includes('rate')) && (
            <span className="relative group px-2 py-0.5 text-xs font-semibold rounded-full bg-red-200 dark:bg-red-800 text-red-800 dark:text-red-200">
              {message.toLowerCase().includes('safety') || message.toLowerCase().includes('blocked') ? 'Safety Blocked' : 'Service Issue'}
