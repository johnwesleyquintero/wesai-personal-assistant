...
- **API Service Interaction**: Interactions with external services like Gemini are encapsulated within `services/*` files. Specifically, patterns are established in [`services/geminiService.ts`](services/geminiService.ts) for Gemini API interactions. Data persistence is handled via LocalStorage in the store.
